...
(function ($) {
    ...
    $.jWindowCrop = function (image, options) {
        ...
        base.init = function () {
            ...
            base.$image.addClass('jwc_image').wrap('<div id="jwc_frame" class="jwc_frame" />'); // wrap image in frame
        ...
        base.setZoom = function (percent) {           
            ...
            if (base.options.zoomPercent > 0) {
                base.workingPercent = base.options.zoomPercent;
                percent = base.options.zoomPercent
            }
            else {
        ...
        base.zoomIn = function () {            
            base.options.zoomPercent = 0;
            var zoomIncrement = (1.0 - base.minPercent) / (base.options.zoomSteps - 1);            
            var workingPercent = parseFloat(base.workingPercent) + parseFloat(zoomIncrement);           
            base.setZoom(workingPercent);
            return false;
        };
        ...
        function initializeDimensions() {
            ...
            if (base.originalWidth > 0) {
                ...
                var x = Math.round(base.originalWidth / 2);
                var y = Math.round(base.originalHeight / 2);
                x = base.options.left;
                y = base.options.top;
                base.focalPoint = { 'x': x, 'y': y };
                //                if (base.options.zoomPercent > 0)
                //                    base.minPercent = base.options.zoomPercent;
        ...
        function focusOnCenter() {
            var left = fillContainer((Math.round(base.focalPoint.x * base.workingPercent) * -1), base.$image.width(), base.options.targetWidth);
            var top = fillContainer((Math.round(base.focalPoint.y * base.workingPercent) * -1), base.$image.height(), base.options.targetHeight);
