...
{        
    ...
    var wrap = $('<div id="'+ settings.containerid +'">').css(
    {
        width: (settings.defaultWidth) ? settings.defaultWidth : obj.width(),
        height: obj.height(),
        position: "absolute"
    });
		...
		$(window).scroll(function()
		{
			...
			else if(scroll > settings.end) 
			{
			   ...
			   // top: settings.end
				...
				if(settings.outOfRange) settings.outOfRange();
			}
			...
			else
			{
				var newleft = $(window).scrollLeft(),
				    ...
				    scroll_container = $("#left-scroll-container");
				//Put back to fixed
				$(obj).css({
				position : "fixed",
				top: settings.margin});
				...
				if(settings.stillInRange) settings.stillInRange();
				if($(obj).attr("class") == "scroll")
				{
					if(newleft > 0)
					{
						$(obj).css({
							position: 'absolute',
							top: newtop + settings.margin});
					}
					else	
					{
						$(obj).css({
			...
			/*
			if($("#left-scroll-container .scroll").css('position') != "absolute")
			{
				var scroll_container = $("#left-scroll-container");
				if($(".content").width() > $(window).width())
				{
					var newleft = -$(window).scrollLeft();
					//scroll_container.css({'left': newleft});
				}
				else
				{
					$("#left-scroll-container").css('left', 'auto');
				}	
			}
			else
			{
