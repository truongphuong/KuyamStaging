 ...
 */ (function ($) {
    $.fn.bjqs = function (options) {
		...
		function custompos(pos)
		{
			if(slidePosition == pos) return;
			$slider.animate({
        ...
        if (settings.showControls && slideCount > 1) {
            ...
            var $controlContainer = $('<ul class="bjqs-controls"></ul>'),
                $next = $('<li><a href="#" class="bjqs-next" title="'+ settings.nextText + '" class="controls">' + settings.nextText + '</a></li>'),
                $previous = $('<li><a href="#" class="bjqs-prev" title="'+ settings.prevText + '" class="controls">' + settings.prevText + '</a></li>');
