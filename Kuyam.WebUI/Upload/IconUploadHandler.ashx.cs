...
namespace Kuyam.WebUI.Upload
{
    ...
    public class IconUploadHandler : IHttpHandler, IRequiresSessionState
    {
        ...
        {
            var statuses = new FilesStatus();
            var headers = context.Request.Headers;

            if (string.IsNullOrEmpty(headers["X-File-Name"]))
            {
                UploadWholeFile(context, out statuses);
            }
            //else
            //{
        ...
        // Upload entire file
        private void UploadWholeFile(HttpContext context, out FilesStatus result)
        {
            result = new FilesStatus();
            for (int i = 0; i < context.Request.Files.Count; i++)
            {
                ...
                result = new FilesStatus(kalturaMediaEntry, mediaid, fullName, file.ContentLength, fullPath);

            }
        }

        private void WriteJsonIframeSafe(HttpContext context, FilesStatus status)
        {
