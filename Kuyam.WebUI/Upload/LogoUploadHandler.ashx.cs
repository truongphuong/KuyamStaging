...
namespace Kuyam.WebUI.Upload
{
    ...
    /// </summary>
    public class LogoUploadHandler : IHttpHandler, IRequiresSessionState

    {
        ...
        private void UploadWholeFile(HttpContext context, List<FilesStatus> statuses)
        {
            for (int i = 0; i < context.Request.Files.Count; i++)
            {
                ...
                //GettyImageService _gettyImageService = EngineContext.Current.Resolve<GettyImageService>();
                //GettyImage gettyImage = new GettyImage
                //{
                //    Status = (int)Types.GettyImageStatus.Downloaded,
                //    GettyImageId = kalturaMediaEntry.Id,
                //    UrlPreview = kalturaMediaEntry.DataUrl,
                //    ProfileId = MySession.ProfileID,
                //    Type = (int)Types.ImageType.Kaltura
                //};
                //_gettyImageService.InsertGettyImage(gettyImage);
                //mediaid = gettyImage.GettyImageId;

                string fullName = Path.GetFileName(file.FileName);
                statuses.Add(new FilesStatus(kalturaMediaEntry, mediaid.ToString(), fullName, file.ContentLength, fullPath));
