...
{
    /// <summary>
    /// Summary description for CompanyAdminPackageHandler
    /// </summary>
    public class CompanyAdminPackageHandler : IHttpHandler, IRequiresSessionState
    {
        ...
        public CompanyAdminPackageHandler()
        {
        ...
        private void UploadWholeFile(HttpContext context, List<FilesStatus> statuses)
        {
            for (int i = 0; i < context.Request.Files.Count; i++)
            {
                ...
                KalturaMediaEntry kalturaMediaEntry = KalturaService.StartSessionAndUploadMedia(_fileStream,
                                                                                                Kaltura.KalturaMediaType
                                                                                                    .IMAGE, fileName);
                _fileStream.Flush();
                _fileStream.Close();
                DeleteFile(fullPath);
                ...
                if (kalturaMediaEntry.Status == Kaltura.KalturaEntryStatus.READY && profile != null)
                {
                    CompanyPackageImage companyPackageImage =
                        _companyProfileService.GetSoonestPackageImageByProfileId(profile.ProfileID);
                    if (companyPackageImage == null)
                        _companyProfileService.CreateCompanyPackageImage(profile.ProfileID, kalturaMediaEntry.Id);
                    else
                    {
                        _companyProfileService.UpdateCompanyPackageImage(companyPackageImage.CompanyPackageImageId, kalturaMediaEntry.Id);
                    }
                }                
